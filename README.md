...
> 面向**非开发用户**：用自然语言告诉 Claude 你要什么，Claude 采访几个选项后自动完成，你不用碰命令行。

---

## 这是什么？

一个 Claude Code Skill + 配套命令行工具（`scripts/flowmax.py`，纯 Python 标准库，无需安装依赖）。

装上 / 拿到之后，你可以这样和 Claude 对话：

```
> 帮我建一个 BTC 的 PM agent，保守一点
...
Claude 会调用本 skill 的脚本，用**你的 JWT** 访问市场后端，拿到真实数据 / 执行真实操作，而不是凭空编造。
...
| **创建 PM Agent** | 决策型交易 agent，自动建 mock 纸面凭证（**不碰真钱**）、组装策略 prompt + 风险配置、默认自动启动 |
...
登录 [Hubble 市场](https://market.dev.gcp.hubble-rpc.xyz) 网站后，从浏览器 **localStorage / cookie** 拿到登录 JWT（Privy）。

> ⚠️ **安全**：JWT 等同于你的账号登录态。**只在你自己的终端里设置环境变量，绝不要贴进任何对话 / 聊天 / 截图。**

```bash
export FLOWMAX_JWT="<把你的 token 粘到这里>"
```

### 2. 环境（可选）

默认连 **staging**（`market.dev`）。要操作**线上正式环境**，再加一条：

```bash
export FLOWMAX_BASE="https://market.prod.gcp.hubble-rpc.xyz"
```

| 环境 | 地址 | 用途 |
|------|------|------|
| staging（默认） | `https://market.dev.gcp.hubble-rpc.xyz` | 测试 / 日常 |
| prod | `https://market.prod.gcp.hubble-rpc.xyz` | 线上真实 |
...
### 方式 A：作为 Claude Code Skill（推荐，非开发用户）

把本目录放进 Claude Code 能识别的 skills 路径（项目级 `.claude/skills/` 或用户级 `~/.claude/skills/`），然后在 Claude Code 里直接用自然语言对话即可。Claude 会：

1. 问你几个高层选项（标的 / 策略目标 / 风险档位 / 是否公开）；
2. **写操作前向你复述并确认**；
3. 调 `scripts/flowmax.py` 完成创建 / 查询 / 运维。

### 方式 B：直接用命令行

```bash
export FLOWMAX_JWT="<你的 JWT>"

# 探测当前环境合法值（asset_type / LLM / mock id / 内嵌模板）
python3 scripts/flowmax.py probe

# 建一个 BTC 保守型 PM（自动 mock 凭证 + 自动启动 + 硬止损）
...
# 看每日盈亏 / 持仓
python3 scripts/flowmax.py logs pnl-summary --agent <agent_id>
python3 scripts/flowmax.py logs positions --agent <agent_id>
```
...
> JWT 从 `FLOWMAX_JWT` 读取；base 从 `FLOWMAX_BASE` 读取（默认 staging）。所有写操作请先确认。
...
| 命令 | 说明 |
|------|------|
| `research create --name --prompt --asset-type --analysis-type [--public] [--wait 120]` | 建单个（也支持 `--spec file.json` 批量） |
...
| 命令 | 说明 |
|------|------|
| `pm create --name --symbol --goal --style [--interval] [--overlay] [--research-ids] [--public]` | 建 PM（自动 mock 凭证 + 组装 prompt/risk + 默认自启） |
...
- 不要把 JWT 写进任何文件、提交进 git、或贴进对话。
- 写操作（创建 / 删除 / 启停）Claude 会先向用户复述确认。
...
flowmax-skill/
├── README.md            # 本文
├── SKILL.md             # Claude Code skill 定义（Claude 读取的指令）
└── scripts/
